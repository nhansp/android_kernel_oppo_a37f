...
/*
 * BRK instruction used as a fault/sentinel by the A64 instruction generators.
 * A generator returns this when it cannot encode the requested instruction;
 * the eBPF JIT also fills unused image slots with it. (mainline: asm/insn-def.h,
 * AARCH64_BREAK_MON | (FAULT_BRK_IMM << 5)).
 */
#define	AARCH64_BREAK_FAULT		0xd4207d00
...
enum aarch64_insn_hint_op {
	...
	/* BTI hints (unused on this target; kept so the JIT's A64_BTI_* macros
	 * compile under the compile-time-false CONFIG_ARM64_BTI_KERNEL guard). */
...
/* A64 instruction generators backported for the eBPF JIT. */
...
/*
 * Cortex-A53 (ARMv8.0) has no LSE atomics, and this tree has no
 * CONFIG_ARM64_LSE_ATOMICS. Provide the no-LSE stub so A64_STADD compiles;
 * the JIT uses the LL/SC (LDXR/STXR) path instead.
 */
static inline
